...
// Send contact/review query forms to store email
const queryMailTo = "highdigitech1906@gmail.com";
const queryMailEndpoint = (() => {
  const currentPath = window.location.pathname;
  const baseUrl = currentPath.includes('/anvogue-html/') 
    ? window.location.origin + '/anvogue-html/'
    : window.location.origin + '/';
  return baseUrl + 'send-product-query.php';
})();
...
const buildQueryMailBody = (form) => {
  ...
  fields.forEach((field) => {
    ...
    if (value) {
      bodyLines.push(`${getFieldLabel(field)}: ${value}`);
    }
  });
...
queryForms.forEach((form) => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const subjectLabel = submitBtn ? submitBtn.textContent.trim() : "Website Query";
    const subject = `${subjectLabel || "Website Query"} - ${document.title}`;
    const body = buildQueryMailBody(form);
    ...
    fields.forEach((field) => {
      ...
      if (value) {
        payload.append(getFieldLabel(field), value);
      }
    });
...
let currentProductQuery = {};

const sendQueryMail = async (payload) => {
  const response = await fetch(queryMailEndpoint, {
    method: "POST",
    headers: {
...
const styleProductQueryButtons = () => {
  document.querySelectorAll(".product-query-btn").forEach((btn) => {
    btn.style.display = "inline-flex";
    btn.style.alignItems = "center";
    btn.style.justifyContent = "center";
    btn.style.minHeight = "52px";
    btn.style.width = btn.classList.contains("button-main") ? "100%" : "";
    btn.style.visibility = "visible";
    btn.style.opacity = "1";
  });
};

const getProductQueryValue = (container, selector) => {
...
const buildProductQueryData = (btn) => {
  const productContainer =
    ...
    btn.closest(".product-detail") ||
    btn.closest(".modal-quickview-main") ||
    document;
  const productName =
    getProductQueryValue(productContainer, ".product-name") ||
    getProductQueryValue(productContainer, ".name") ||
    document.title;
...
const getProductQueryModal = () => {
  let modal = document.querySelector(".product-query-modal");
  ...
  modal.className = "product-query-modal";
  modal.style.cssText =
    "position:fixed;inset:0;z-index:999;background:rgba(0,0,0,.55);display:none;align-items:center;justify-content:center;padding:20px;";
  modal.innerHTML = `
    <div class="product-query-panel bg-white rounded-xl w-full" style="max-width:560px;max-height:calc(100vh - 40px);overflow:auto;">
      <div class="flex items-center justify-between gap-4 py-5 px-6 border-b border-line">
        <div class="heading5">Product Query</div>
        <button type="button" class="close-product-query w-8 h-8 flex items-center justify-center rounded-full border border-line">
          <i class="ph ph-x text-sm"></i>
        </button>
      </div>
      <form class="product-query-form p-6">
        <div class="grid sm:grid-cols-2 grid-cols-1 gap-4 gap-y-5">
          <div class="sm:col-span-2">
            <label class="caption1 text-secondary2 block mb-2" for="product-query-item">Item Name</label>
            <input class="product-query-item border-line px-4 py-3 w-full rounded-lg bg-surface" id="product-query-item" name="Item Name" type="text" readonly />
          </div>
          <div>
            <label class="caption1 text-secondary2 block mb-2" for="product-query-name">Name *</label>
            <input class="product-query-name border-line px-4 py-3 w-full rounded-lg" id="product-query-name" name="Name" type="text" required />
          </div>
          <div>
            <label class="caption1 text-secondary2 block mb-2" for="product-query-phone">Phone *</label>
            <input class="product-query-phone border-line px-4 py-3 w-full rounded-lg" id="product-query-phone" name="Phone" type="tel" required />
          </div>
          <div>
            <label class="caption1 text-secondary2 block mb-2" for="product-query-email">Email</label>
            <input class="product-query-email border-line px-4 py-3 w-full rounded-lg" id="product-query-email" name="Email" type="email" />
          </div>
          <div>
            <label class="caption1 text-secondary2 block mb-2" for="product-query-pin">Pin Code *</label>
            <input class="product-query-pin border-line px-4 py-3 w-full rounded-lg" id="product-query-pin" name="Pin Code" type="text" inputmode="numeric" required />
          </div>
          <div class="sm:col-span-2">
            <label class="caption1 text-secondary2 block mb-2" for="product-query-address">Full Address *</label>
            <textarea class="product-query-address border-line px-4 py-3 w-full rounded-lg" id="product-query-address" name="Full Address" rows="4" required></textarea>
          </div>
          <div class="sm:col-span-2">
            <label class="caption1 text-secondary2 block mb-2" for="product-query-message">Additional Details</label>
            <textarea class="product-query-message border-line px-4 py-3 w-full rounded-lg" id="product-query-message" name="Additional Details" rows="3"></textarea>
          </div>
        </div>
        <div class="product-query-status caption1 mt-4" style="display:none;"></div>
        <button type="submit" class="product-query-submit button-main w-full text-center mt-6" style="display:flex;align-items:center;justify-content:center;width:100%;min-height:52px;border:0;">
          <span class="submit-text">Send Query</span>
          <span class="submit-loader" style="display:none;width:18px;height:18px;border:2px solid rgba(255,255,255,.5);border-top-color:#fff;border-radius:50%;animation:productQuerySpin .8s linear infinite;"></span>
        </button>
      </form>
    </div>
  `;
  ...
  if (!document.querySelector("#product-query-loader-style")) {
    const loaderStyle = document.createElement("style");
    loaderStyle.id = "product-query-loader-style";
    loaderStyle.innerHTML = "@keyframes productQuerySpin{to{transform:rotate(360deg)}}";
  ...
  modal.querySelector(".product-query-panel").addEventListener("click", (e) => {
    e.stopPropagation();
  });
  modal.querySelector(".close-product-query").addEventListener("click", closeModal);
  modal.querySelector(".product-query-form").addEventListener("submit", async (e) => {
    e.preventDefault();

    const form = e.currentTarget;
    const submitBtn = form.querySelector(".product-query-submit");
    const submitText = form.querySelector(".submit-text");
    const submitLoader = form.querySelector(".submit-loader");
    const status = form.querySelector(".product-query-status");
    ...
    const bodyLines = [
      ...
      "",
      `Item Name: ${currentProductQuery.productName || formData.get("Item Name")}`,
      currentProductQuery.productPrice ? `Price: ${currentProductQuery.productPrice}` : "",
      currentProductQuery.quantity ? `Quantity: ${currentProductQuery.quantity}` : "",
      currentProductQuery.color ? `Color: ${currentProductQuery.color}` : "",
      currentProductQuery.size ? `Size: ${currentProductQuery.size}` : "",
      "",
      "Customer details:",
      `Name: ${formData.get("Name")}`,
      `Phone: ${formData.get("Phone")}`,
      formData.get("Email") ? `Email: ${formData.get("Email")}` : "",
      `Full Address: ${formData.get("Full Address")}`,
      `Pin Code: ${formData.get("Pin Code")}`,
      formData.get("Additional Details") ? `Additional Details: ${formData.get("Additional Details")}` : "",
      "",
      `Page: ${window.location.href}`,
    ].filter(Boolean);
    const subject = `Product Query - ${currentProductQuery.productName || formData.get("Item Name")}`;
    const payload = new FormData();

    payload.append("Subject", subject);
    payload.append("Item Name", currentProductQuery.productName || formData.get("Item Name"));
    payload.append("Price", currentProductQuery.productPrice || "");
    payload.append("Quantity", currentProductQuery.quantity || "");
    payload.append("Color", currentProductQuery.color || "");
    payload.append("Size", currentProductQuery.size || "");
    payload.append("Name", formData.get("Name"));
    payload.append("Phone", formData.get("Phone"));
    payload.append("Email", formData.get("Email") || "");
    payload.append("Full Address", formData.get("Full Address"));
    payload.append("Pin Code", formData.get("Pin Code"));
    payload.append("Additional Details", formData.get("Additional Details") || "");
    payload.append("Page", window.location.href);
    payload.append("Message", bodyLines.join("\n"));

    submitBtn.disabled = true;
    submitText.style.display = "none";
    submitLoader.style.display = "inline-block";
    status.style.display = "block";
    status.style.color = "#696C70";
    status.textContent = "Sending your query...";

    try {
      await sendQueryMail(payload);
      status.style.color = "#3DAB25";
      status.textContent = "Query sent successfully.";
      ...
      status.style.color = "#DB4444";
      status.textContent = "Could not send right now. Please check your internet connection and try again.";
      submitBtn.disabled = false;
      submitLoader.style.display = "none";
      submitText.style.display = "inline";
      submitText.textContent = "Send Query";
...
const openProductQueryModal = (btn) => {
  const modal = getProductQueryModal();
  const form = modal.querySelector(".product-query-form");
  const submitBtn = form.querySelector(".product-query-submit");
  const submitText = form.querySelector(".submit-text");
  const submitLoader = form.querySelector(".submit-loader");
  const status = form.querySelector(".product-query-status");

  currentProductQuery = buildProductQueryData(btn);
  form.reset();
  submitBtn.disabled = false;
  submitText.style.display = "inline";
  submitText.textContent = "Send Query";
  submitLoader.style.display = "none";
  status.style.display = "none";
  modal.querySelector(".product-query-item").value = currentProductQuery.productName;
  modal.style.display = "flex";
  modal.querySelector(".product-query-name").focus();
};

styleProductQueryButtons();

document.addEventListener(
  "click",
  (e) => {
    const btn = e.target.closest(".product-query-btn");
...
// Banner carousel
if (document.querySelector(".swiper-banner-carousel")) {
  document.querySelectorAll(".swiper-banner-carousel").forEach((slider) => {
    new Swiper(slider, {
      loop: true,
      speed: 800,
      autoplay: {
...
// Dairy video card
document.querySelectorAll(".dairy-video-card").forEach((videoCard) => {
  videoCard.addEventListener("click", () => {
    const youtubeId = videoCard.getAttribute("data-youtube-id");

    if (!youtubeId || youtubeId === "VIDEO_ID") {
      return;
    }

    const iframe = document.createElement("iframe");
    iframe.src = `https://www.youtube.com/embed/${youtubeId}?autoplay=1&rel=0`;
    iframe.title = videoCard.getAttribute("aria-label") || "Dairy farm video";
    iframe.allow =
